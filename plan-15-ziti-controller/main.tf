...
provider "helm" {
  ...
  kubernetes {
    host                   = yamldecode(base64decode(data.tfe_outputs.k8s_state.values.kubeconfig)).clusters[0].cluster.server
    token                  = yamldecode(base64decode(data.tfe_outputs.k8s_state.values.kubeconfig)).users[0].user.token
    cluster_ca_certificate = base64decode(yamldecode(base64decode(data.tfe_outputs.k8s_state.values.kubeconfig)).clusters[0].cluster.certificate-authority-data)
  }
}
...
module "ziti_controller" {
  ...
  dns_zone                = data.tfe_outputs.k8s_state.values.dns_zone
...
resource "helm_release" "ziti_console" {
  ...
  namespace  = data.tfe_outputs.k8s_state.values.ziti_namespace
  ...
  values = [yamlencode({
    ingress = {
      ...
      annotations = {
        "cert-manager.io/cluster-issuer" = data.tfe_outputs.k8s_state.values.cluster_issuer_name
      }
      advertisedHost = "console.${data.tfe_outputs.k8s_state.values.dns_zone}"
      tlsSecret      = "${var.ziti_console_release}-tls-secret"
    }
    settings = {
      edgeControllers = [{
        name    = "Ziti Edge Mgmt API"
        url     = "https://${var.ziti_controller_release}-mgmt.${data.tfe_outputs.k8s_state.values.ziti_namespace}.svc:443"
        default = "true"
      }]
    }
  })]
